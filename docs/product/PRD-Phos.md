...
| TBD  | 백엔드 엔지니어링 | 세션, 렌더, 자산 전달, 삭제/내보내기 처리        |
...
- 앱 내 결과 화면에서 결과물 확인과 저장 흐름이 단순해야 한다.
...
- 불변조건:
  - 세션 삭제 확정 시 해당 세션 자산은 즉시 접근 불가 상태가 되어야 함
...
- 결과물 저장 흐름은 불안정한 네트워크에서도 높은 성공률을 유지할 수 있다.
...
- **1단계**: 세션/자산/동의 리소스 모델 확정, 보관/삭제 불변조건 확정, 이벤트 스키마 고정
- **2단계**: 프레임 선택, Live Booth 촬영, 메이킹 영상 기록, 저사양 프리셋 구현
- **3단계**: 컷 선택/간단 편집, 최종 렌더링, 결과 화면/로컬 저장 구현
...
- analytics vendor는 추후 확정하되, MVP 이벤트 이름은 아래로 고정한다:
  `session_started`, `camera_ready`, `capture_completed`, `render_succeeded`, `result_view_opened`, `local_save_tapped`, `local_save_succeeded`, `consent_updated`, `export_requested`, `export_completed`, `deletion_requested`, `deletion_completed`
- 익명 식별은 MVP에서 `sessionId` 기준으로 정의한다
- 결과물 접근 가능 시간은 항상 `retentionExpiresAt` 이내로 제한한다
